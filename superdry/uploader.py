...
class ProductUploader():
    ...
    def login(self):
        ...
        # data = {
        #     "companyName": "Dev",
        #     "email": "dev@greentriangleuk.com",
        #     "password": "Helo*1234"
        # }
        data = {
            "companyName": "superdry",
            "email": "accounts@greentriangleuk.com",
            "password": "Superdry@1234"
        }
        json_data = json.dumps(data) 
        resp = self.session.post(f"{API_URL}{API_ENDPOINT_LOGIN}", data=json_data, headers=headers)
        respData = resp.json()
    ...
    def addSku(self, data):
        headers = {'Content-Type': 'application/json'} 
        json_data = json.dumps(data) 
        resp = self.session.post(f"{API_URL}{API_ENDPOINT_ADDSKU}", data=json_data, headers=headers)
        if resp.status_code == 200:
    ...
    def addSkuImages(self, skuId, imageNames):
        ...
        resp = self.session.post(f"{API_URL}{API_ENDPOINT_ADDSKUIMAGE}", files=files, params={"SkuId": skuId,})
        if resp.status_code == 200:
            return True
